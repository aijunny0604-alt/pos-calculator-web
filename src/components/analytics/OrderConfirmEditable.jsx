...
export default function OrderConfirmEditable({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm overflow-y-auto">
      <div
        className="movis-glass-card w-full max-w-lg p-4 sm:p-5 my-8 min-w-0"
        style={{ background: 'rgba(10, 25, 41, 0.96)' }}
      >
        {/* 헤더 */}
        <div className="flex items-center gap-2 mb-3 movis-text-primary">
          <Truck className="w-5 h-5 text-[var(--jarvis-cyan)]" />
          <h3 className="text-lg font-bold flex-1">주문 등록 확인</h3>
          {!editMode && initialParams.needsConfirmation && (
            <button
              type="button"
        ...
        {pending.warnings.length > 0 && (
          <div className="bg-amber-500/10 border border-amber-400/30 rounded-lg p-3 mb-3 space-y-1">
        ...
        {/* === 거래처 섹션 === */}
        <div className="mb-3 p-3 rounded-lg" style={{ background: 'rgba(15,26,45,0.5)', border: '1px solid rgba(0,212,255,0.2)' }}>
          <div className="text-xs font-mono uppercase tracking-wider mb-2" style={{ color: 'var(--jarvis-text-muted)' }}>거래처</div>
          {!editMode ? (
            <div className="text-sm" style={{ color: 'var(--jarvis-text-primary)' }}>
              {initialParams.customerName} {!customerId && <span className="text-amber-400 text-xs">❓미확정</span>}
            </div>
          ) : (
            <div className="space-y-2">
              {/* 현재 선택된 거래처 */}
              {customerId ? (
                <div className="text-sm flex items-center gap-2" style={{ color: '#00ff88' }}>
                  ✓ {customerName} {customerPhone && <span className="text-xs opacity-70">· {customerPhone}</span>}
                  <button onClick={() => { setCustomerId(null); setAllowNewCustomer(false); }} className="ml-auto text-xs opacity-70 hover:opacity-100">변경</button>
                </div>
              ) : allowNewCustomer ? (
                <div className="text-sm flex items-center gap-2" style={{ color: '#ffaa00' }}>
                  ⚠ 신규 등록: "{customerName}"
                  <button onClick={() => setAllowNewCustomer(false)} className="ml-auto text-xs opacity-70 hover:opacity-100">취소</button>
                </div>
              ) : (
                <div className="text-sm" style={{ color: '#ff4d6d' }}>
                ...
                <>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-2 py-1.5 rounded text-xs font-mono"
                  ...
                  {showCandidatesInline && (
                    <div className="space-y-1">
                      <div className="text-[10px] opacity-70 font-mono">💡 혹시 이거?</div>
                      {initialParams.customerCandidates.map((c) => (
                        <button
                          key={c.id}
                          onClick={() => pickCustomer(c)}
                          className="w-full text-left px-2 py-1.5 rounded text-xs flex items-center justify-between hover:bg-cyan-500/15"
                          style={{ background: 'rgba(0,212,255,0.08)', border: '1px solid rgba(0,212,255,0.2)' }}
                        >
                          <span style={{ color: '#e8f4fd' }}>{c.name}{c.phone && <span className="opacity-60"> · {c.phone}</span>}</span>
                          <span className="text-[10px] font-mono" style={{ color: '#4dffff' }}>{Math.round(c.score * 100)}% · {c.reason}</span>
                        </button>
                      ))}
                    </div>
                  )}
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {customerSearchResult.candidates.map((c) => (
                        <button
                          ...
                          className="w-full text-left px-2 py-1.5 rounded text-xs hover:bg-cyan-500/15"
                          style={{ background: 'rgba(15,26,45,0.6)', border: '1px solid rgba(0,212,255,0.15)' }}
                        >
                          <span style={{ color: '#e8f4fd' }}>{c.item.name}{c.item.phone && <span className="opacity-60"> · {c.item.phone}</span>}</span>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  >
                    <UserPlus className="w-3 h-3" />신규 거래처로 등록: "{customerQuery || initialParams.customerName}"
                  </button>
                </>
              )}
            </div>
        ...
        <div className="mb-3 p-3 rounded-lg" style={{ background: 'rgba(15,26,45,0.5)', border: '1px solid rgba(0,212,255,0.2)' }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs font-mono" style={{ color: 'var(--jarvis-text-muted)' }}>
            ...
            {items.map((it, idx) => (
              <div key={idx} className="text-xs">
                <div className="flex items-center gap-2">
                  <div className="flex-1 min-w-0 truncate" style={{ color: it.zeroPrice ? '#ff4d6d' : '#e8f4fd' }}>
                    {it.autoMatched && <span className="text-amber-400 mr-1">🔄</span>}
                    {it.zeroPrice && <span className="text-rose-400 mr-1">⚠️</span>}
                    {it.name}
                    {it.originalInput && <span className="opacity-50 ml-1">["{it.originalInput}"]</span>}
                  </div>
                  {editMode && (
                    <button
                ...
                {editMode ? (
                  <div className="flex items-center gap-1.5 mt-1">
                    <span className="text-[10px] opacity-60 w-8">수량</span>
                    <input
                      type="number" min="1"
                      value={it.quantity}
                      onChange={(e) => setItems((prev) => prev.map((x, i) => i === idx ? { ...x, quantity: Math.max(1, parseInt(e.target.value || '1', 10)) } : x))}
                      className="w-14 px-1.5 py-0.5 rounded text-xs font-mono"
                      style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(0,212,255,0.3)', color: '#e8f4fd' }}
                    />
                    <span className="text-[10px] opacity-60 ml-2 w-8">단가</span>
                    <input
                      type="number" min="0"
                      value={it.price}
                      ...
                      className="w-20 px-1.5 py-0.5 rounded text-xs font-mono"
                    ...
                    />
                    <span className="text-[10px] opacity-60 ml-auto">= {fmtNum(it.price * it.quantity)}원</span>
                    <button
                      onClick={() => setItems((prev) => prev.filter((_, i) => i !== idx))}
                      className="text-rose-400 opacity-70 hover:opacity-100 text-[10px]"
                    >제거</button>
                  </div>
                ) : (
                  <div className="text-[11px] opacity-70 mt-0.5">
                    × {it.quantity} = {fmtNum(it.price * it.quantity)}원 (단가 {fmtNum(it.price)})
                  </div>
                )}

                {/* 제품 검색 dropdown */}
                {editMode && productSearchIdx === idx && (
                  <div className="mt-1.5 p-1.5 rounded space-y-1" style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(0,212,255,0.3)' }}>
                    <input
                      type="text"
                      value={productQuery}
                    ...
                    {it.alternatives?.length > 0 && (
                      <div className="text-[10px] opacity-70">대체 후보:</div>
                    )}
                    {it.alternatives?.map((alt) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full text-left px-2 py-1 rounded text-[11px] hover:bg-cyan-500/15"
                          style={{ background: 'rgba(0,212,255,0.06)' }}
                        >
                          {p.name} <span className="opacity-60">({fmtNum(p[priceType] || p.wholesale)}원)</span>
                        </button>
                      );
                    })}
                    {/* 검색 결과 */}
          ...
          <div className="mt-2 pt-2 border-t border-cyan-400/15 flex items-center justify-between text-sm">
            <span className="text-xs opacity-70">합계 (VAT 포함)</span>
            <span className="font-bold" style={{ color: '#4dffff' }}>{fmtNum(total)}원</span>
          </div>
        </div>
        ...
        {editMode && (
          <div className="mb-3">
            <label className="text-xs font-mono uppercase tracking-wider flex items-center gap-1.5 mb-1" style={{ color: 'var(--jarvis-text-muted)' }}>
              <MessageSquare className="w-3 h-3" />추가 지시사항 / 메모
            </label>
            <textarea
              value={memo || ''}
        ...
        {/* 안내 */}
        {!canConfirm && (
          <div className="text-[10px] mb-2 px-2 py-1.5 rounded" style={{ background: 'rgba(255,77,109,0.1)', color: '#ff4d6d', border: '1px solid rgba(255,77,109,0.3)' }}>
        ...
        <div className="flex gap-2 mt-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg font-semibold disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <div className="text-[10px] opacity-50 mt-2 text-center">
          💡 [수정] 모드에서 거래처/제품/수량/단가/메모를 직접 편집할 수 있어요.
        </div>
      </div>
    </div>
  );
