  ...
  try {
    switch (name) {
      ...
      case 'getProductInfo': {
        ...
        if (!target) {
          const candidates = findProductCandidates(q, products).slice(0, 5);
          return {
            ok: true,
            data: {
              found: false,
              ...
              candidates: candidates.map((c) => ({ name: c.name, category: c.category, stock: c.stock })),
            },
          };
        }
        // 최근 30일 판매량 집계
...
- **"스덴 밴딩 54-30 재고 알려줘" / "X 재고 있어?" / "Y 얼마야?" / "Z 정보" → 무조건 getProductInfo 호출** (단일 제품 fuzzy 매칭)
...
### 거래처/주문 조회
- **"강남오토 정보" / "X 매출 얼마?" / "Y 최근 언제 왔어?" → getCustomerInfo** (단일 거래처 1곳)
