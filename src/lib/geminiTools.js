...
function buildPendingAction(name, args, { customers, products, aiLearningData = [] }) {
  ...
  if (name === 'saveOrder') {
    ...
    for (const it of items) {
      ...
      if (!pname) continue;
      let product = findProductSmart(pname, products, aiLearningData);
      let autoMatched = false;
      let originalInput = null;
      if (!product) {
        // 🎯 candidates top1 자동 매칭 (정확도 boost)
        const candidates = findProductCandidates(pname, products);
        if (candidates.length > 0) {
          const top1Name = candidates[0];
          product = products.find((p) => p.name === top1Name);
          if (product) {
            autoMatched = true;
            originalInput = pname;
            autoMatchedDetails.push({ input: pname, matched: product.name, alternatives: candidates.slice(1, 4) });
          }
        }
        if (!product) {
