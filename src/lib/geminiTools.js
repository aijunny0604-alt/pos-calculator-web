...
export const GEMINI_TOOLS = [
  ...
  {
    name: 'getDeadStock',
    description: '안 나가는 재고(데드스톡) — 재고는 있는데 최근 N개월간 판매가 0건인 제품을 묶인 금액(재고×도매가) 큰 순으로. "안 나가는 재고", "재고만 쌓인 거", "묵은 재고", "안 팔리는 제품", "돈 묶인 재고" 같은 질문에 사용. 정리·할인·반품 검토 대상 식별.',
    parameters: {
      type: 'object',
      properties: {
        months: { type: 'integer', description: '판매 없음 기준 개월수 (기본 3)' },
        minStock: { type: 'integer', description: '최소 재고 (기본 1 — 재고 있는 것만)' },
        limit: { type: 'integer', description: 'TOP N (기본 20)' },
      },
    },
  },
  ...
  {
    name: 'bulkUpdateProductsByCondition',
    description: '조건에 맞는 여러 제품의 재고 또는 가격을 한 번에 변경. "재고 10개 미만 전부 30개로 맞춰", "○○ 카테고리 전부 5% 인상", "스덴밴딩 들어간 거 다 10개씩 채워" 같은 조건부 일괄 요청에 사용. 조건으로 대상을 자동 선별해 확인모달로 미리보기 후 적용(대상이 넓을 수 있어 반드시 confirm). 액션은 재고(setStock/addStock) 또는 가격(pricePercent) 중 하나만 지정.',
    parameters: {
      type: 'object',
      properties: {
        category: { type: 'string', description: '이 카테고리 제품만 대상 (선택, 부분일치)' },
        nameContains: { type: 'string', description: '제품명에 이 단어가 포함된 것만 대상 (선택)' },
        stockBelow: { type: 'number', description: '현재 재고가 이 값 "미만"인 제품만 (선택)' },
        stockAtMost: { type: 'number', description: '현재 재고가 이 값 "이하"인 제품만 (선택)' },
        allProducts: { type: 'boolean', description: '조건 없이 전체 제품 대상 (⚠️위험 — 사용자가 "전부/모든 제품"을 명시할 때만 true)' },
        setStock: { type: 'number', description: '재고를 이 값으로 설정 (예: 30 → 전부 30개로)' },
        addStock: { type: 'number', description: '재고를 이만큼 증감 (예: 10 → 각각 +10, -5 → 각각 -5)' },
        pricePercent: { type: 'number', description: '가격 % 조정 (예: 5=5% 인상, -10=10% 인하). priceType 필수' },
        priceType: { type: 'string', enum: ['wholesale', 'retail'], description: 'pricePercent 사용 시 대상 가격 (wholesale=도매가, retail=소비자가)' },
      },
    },
  },
  {
    name: 'navigatePage',
    description: '앱의 특정 페이지로 화면을 이동합니다(부작용 없음, 바로 실행). "송장 페이지 열어줘", "관리자 가줘", "명세서 열어줘", "재고 현황 보여줘", "스토어 주문 열어", "주문내역 가줘", "거래처 관리 열어" 같은 요청에 사용. page는 반드시 아래 중 하나: dashboard(대시보드), pos(제품 주문), orders(주문 내역), saved-carts(저장 장바구니), customers(거래처 관리), stock(재고 현황), burnway-stock(번웨이 재고), shipping(택배 송장), invoices(명세서), memos(메모 모아보기), ai-order(AI 주문인식), smartstore(스마트스토어 주문), admin(관리자). 단순 조회/분석이면 이동하지 말고 해당 조회 도구를 쓸 것.',
    parameters: {
      type: 'object',
      properties: {
        page: { type: 'string', description: '이동할 페이지 키(위 목록 중 하나)' },
      },
      required: ['page'],
    },
  },
  {
    name: 'getDailyClose',
    description: '오늘 하루 마감 요약을 한 번에 보여줍니다. "오늘 정리해줘", "오늘 마감", "오늘 어땠어", "하루 요약" 같은 요청에 사용. 오늘 주문 건수·매출, 입금, 반품, 재고 부족, 스토어 미처리 건수를 종합. 읽기 전용.',
    parameters: { type: 'object', properties: {} },
  },
  {
    name: 'draftMessage',
    description: '거래처에게 보낼 문자/카톡 메시지 또는 견적서/발주서 초안을 작성해 복사 가능한 카드로 보여줍니다. "○○ 미수 정산 안내문자", "재구매 제안 문구", "발주 확인 문자", "○○ 견적서 문구 만들어줘", "부족한 재고 발주서 뽑아줘" 같은 요청에 사용. 반드시 먼저 필요한 데이터를 조회(미수액=getCustomerInfo/getOverdueCustomers, 재구매후보=getNextBestOffers, 회수문구=getCollectionPlan, 견적/발주 제품·단가=searchProducts/getProductInfo, 발주추천=getRestockRecommendations)한 뒤, 실제 숫자·이름·품목·단가를 넣어 완성해 message에 전달. 견적서/발주서는 [품목 / 수량 / 단가 / 금액] 목록 + 합계(공급가/부가세 포함 여부 명시)를 깔끔한 텍스트 표 형식으로. DB·발송·저장 없음(사장님이 복사해 직접 사용). 존댓말, 정중·간결하게.',
    parameters: {
      type: 'object',
      properties: {
        recipientName: { type: 'string', description: '받는 거래처/고객 이름 (선택)' },
        purpose: { type: 'string', description: '용도 라벨 (예: 미수 정산 안내, 재구매 제안, 발주 확인, 견적서, 발주서)' },
        message: { type: 'string', description: '완성된 본문. 실제 숫자·이름·품목·단가 포함. 문자는 정중한 존댓말, 견적/발주서는 품목표+합계. 이 텍스트가 그대로 복사됨. 과한 이모지/마크다운 없이 바로 쓸 수 있게.' },
      },
      required: ['message'],
    },
  },
...
export function executeTool(name, args = {}, context = {}) {
  ...
  try {
    switch (name) {
      ...
      case 'getDeadStock':
        return { ok: true, data: getDeadStock(products, orders, args) };
      // 고급 분석 5종
      ...
      case 'navigatePage': {
        const PAGE_LABELS = {
          dashboard: '대시보드', pos: '제품 주문', orders: '주문 내역', 'saved-carts': '저장 장바구니',
          customers: '거래처 관리', stock: '재고 현황', 'burnway-stock': '번웨이 재고', shipping: '택배 송장',
          invoices: '명세서', memos: '메모 모아보기', 'ai-order': 'AI 주문인식', smartstore: '스마트스토어 주문', admin: '관리자',
        };
        const page = String(args?.page || '').trim();
        if (!PAGE_LABELS[page]) {
          return { ok: false, error: `이동할 수 없는 페이지입니다: "${page}". (가능: ${Object.keys(PAGE_LABELS).join(', ')})` };
        }
        return { ok: true, data: { __navigate: true, page, label: PAGE_LABELS[page] } };
      }
      case 'getDailyClose': {
        ...
        const todayOrders = (orders || []).filter((o) => kstDate(o.createdAt || o.created_at) === todayKST);
        const orderCount = todayOrders.length;
        const revenue = todayOrders.reduce((s, o) => s + (Number(o.totalAmount || o.total || 0) - Number(o.totalReturned || 0)), 0);
        const todayPayments = (context.paymentHistory || []).filter((h) => kstDate(h.paid_at || h.paidAt) === todayKST);
        const paidToday = todayPayments.reduce((s, h) => s + Number(h.amount || 0), 0);
        const todayReturns = (context.customerReturns || []).filter((r) => kstDate(r.returnedAt || r.returned_at || r.created_at) === todayKST);
        const returnAmt = todayReturns.reduce((s, r) => s + Number(r.total || (Number(r.price || 0) * Number(r.quantity || r.returnQuantity || 0)) || 0), 0);
        const lowStock = (products || []).filter((p) => { const st = Number(p.stock); return Number.isFinite(st) && st <= 3; });
        const storePending = (context.externalOrders || []).filter((o) =>
          o.needs_naver_confirm || o.needs_naver_dispatch ||
          (!o.naver_dispatch_succeeded_at && !o.internal_order_id && !/CANCEL|RETURN|DELIVERED|PURCHASE|shipped|cancelled/i.test(o.order_status || ''))
        ).length;
        const won = (n) => Number(n || 0).toLocaleString('ko-KR');
        return { ok: true, data: {
          date: todayKST,
          orderCount, revenue, paidToday,
          returnCount: todayReturns.length, returnAmt,
          lowStockCount: lowStock.length,
          lowStockTop: lowStock.slice(0, 5).map((p) => `${p.name}(${Number(p.stock) || 0}개)`),
          storePending,
          message: `📅 오늘(${todayKST}) 마감 — 주문 ${orderCount}건 · 매출 ${won(revenue)}원 · 입금 ${won(paidToday)}원 · 반품 ${todayReturns.length}건(${won(returnAmt)}원) · 재고부족 ${lowStock.length}종 · 스토어 미처리 ${storePending}건`,
        } };
      }
      case 'draftMessage': {
        const message = String(args?.message || '').trim();
        if (!message) return { ok: false, error: '메시지 내용이 비어있습니다.' };
        return {
          ok: true,
          data: {
            __messageDraft: true,
            recipientName: String(args?.recipientName || '').trim(),
            purpose: String(args?.purpose || '메시지').trim(),
            message,
          },
        };
      }
...
function buildPendingAction(name, args, { customers, products, aiLearningData = [], orders = [] }) {
  ...
  if (name === 'bulkUpdateProductsByCondition') {
    const { category, nameContains, stockBelow, stockAtMost, allProducts,
            setStock, addStock, pricePercent, priceType } = args;
    // 액션 검증 — 하나만
    const isStockAction = setStock != null || addStock != null;
    const isPriceAction = pricePercent != null;
    if (!isStockAction && !isPriceAction) {
      return { ok: true, data: { __clarification: true, question: '💬 무엇을 바꿀까요? 재고(예: "전부 30개로")인지, 가격(예: "5% 인상")인지 알려주세요.' } };
    }
    if (isStockAction && isPriceAction) {
    ...
    // 대상 선별 (로컬 필터 — 토큰 절약)
    const hasCondition = !!category || !!nameContains || stockBelow != null || stockAtMost != null;
    if (!hasCondition && !allProducts) {
      return { ok: true, data: { __clarification: true, question: '💬 어떤 제품들이 대상인가요? (예: "OO 카테고리", "재고 10개 미만", "스덴밴딩 들어간 거"). 전체가 맞으면 "모든 제품"이라고 해주세요.' } };
    }
    const catLower = (category || '').toLowerCase();
    const nameLower = (nameContains || '').toLowerCase();
    let targets = (products || []).filter((p) => {
      if (category && !(p?.category || '').toLowerCase().includes(catLower)) return false;
      if (nameContains && !(p?.name || '').toLowerCase().includes(nameLower)) return false;
      const st = Number(p?.stock) || 0;
      if (stockBelow != null && !(st < Number(stockBelow))) return false;
      if (stockAtMost != null && !(st <= Number(stockAtMost))) return false;
      return true;
    });
    if (targets.length === 0) {
      return { ok: false, error: '조건에 맞는 제품이 없습니다. 조건(카테고리/이름/재고)을 다시 확인해주세요.' };
    }
    // 안전 상한 — 너무 넓으면 확인 강화 (실행은 가능하되 경고)
    const HARD_CAP = 500;
    if (targets.length > HARD_CAP) {
    ...
    if (isStockAction) {
      const updates = targets.map((p) => {
        ...
        const newStock = setStock != null
          ? Math.max(0, Math.floor(Number(setStock)))
          : Math.max(0, oldStock + Math.floor(Number(addStock)));
        return { productId: p.id, productName: p.name, inputName: p.name, oldStock, newStock, diff: newStock - oldStock };
      });
      ...
      const warnings = [`⚠️ ${changed.length}개 제품 재고를 한 번에 변경합니다. 대상을 꼭 확인하세요.`];
      return { ok: true, data: {
        __pending: true, action: 'bulkUpdateProductStock',
        params: { updates: changed, missing: [], totalIn: changed.filter(u=>u.diff>0).reduce((a,u)=>a+u.diff,0), totalOut: changed.filter(u=>u.diff<0).reduce((a,u)=>a+Math.abs(u.diff),0) },
        warnings,
        preview: `📦 조건부 재고 일괄 변경\n조건: ${condDesc}\n작업: ${actDesc}\n대상: ${changed.length}개\n\n${previewLines}${more}`,
      } };
    }
    ...
    return { ok: true, data: {
      ...
      params: { updates },
      warnings,
      preview: `💰 조건부 ${ptLabel} 일괄 ${pct > 0 ? '인상' : '인하'}\n조건: ${condDesc}\n조정: ${pct > 0 ? '+' : ''}${pct}% (100원 반올림)\n대상: ${updates.length}개\n\n${previewLines}${more}`,
    } };
  }
...
## 🚫🚫🚫 [최우선·절대규칙] 매입 발주 ≠ 재고 변경 (제일 먼저 지킬 것)
사용자가 **"추가 주문 / 발주 / 재주문 / 재고 부족해서 주문 / 들여놔야 / 시켜야겠다 / 발주하려고"**라고 하면,
이건 **공급처(JSR 등)에 물건을 주문(매입 발주)하려는 것**이지 **우리 재고 숫자를 바꾸려는 게 절대 아니다.**
- ✅ 할 일: **getPurchaseStatus**(이미 미입고인지) + **getRestockRecommendations**(판매속도 기반 추천 발주수량)로 **조회만** → "발주 추천 N개 / 발주서 뽑아드릴까요?"를 **글로** 답한다.
- 🚫 **절대 금지**: bulkUpdateProductStock·updateProductStock(재고 숫자 변경) 호출. "재고를 N개로 변경/채울까요?" 제안도 금지.
...
## 🧠 이해도 강화 (질문을 더 똑똑하게 해석)
- **지시대명사·생략 해석(★맥락)**: "그거/그 제품/걔/방금/아까/위에 그거/이전 거/그 업체"는 **직전 대화에서 마지막으로 다룬 제품/거래처/주문을 그대로 대상**으로 삼아라. 예) 앞서 "다운파이프" 얘기 후 "그거 재고 얼마야?"=다운파이프 재고. 대상이 대화에 없으면 그때만 되묻기.
- **후속·생략 질문**: "그럼 소매가는?", "재고는?", "김해는?"처럼 짧게 이어 물으면 **직전 주제/도구를 유지**하고 바뀐 부분만 반영. ("김해는?"=직전 거래처를 'WP튠 김해'로 바꿔 같은 조회)
- **한글 숫자·단위 → 숫자 변환**: 오만원=50000, 십만원=100000, 삼만오천원=35000, 백이십=120, "삼개/세개"=3, "열개"=10, "한 박스/한 세트"=문맥 수량, "반값"=현재가×0.5, "두 배"=×2, "만 단위"=×10000. 반드시 정수로 환산해 인자에 넣어라.
- **읽기 현장표현 → 조회 도구**: "얼마 받을 거 남았어/받을 돈"=미수(getOverdueCustomers/getCustomerInfo), "요즘 안 오는 단골/뜸한 곳"=getDormantCustomers, "이번달 장사 어때/매출 어때"=getCompositeSummary, "제일 많이 사가는 데/큰 손"=getTopCustomers, "마진 짜게 남는/손해보는"=getMarginLeakage, "곧 떨어질/모자란 거"=getLowStockProducts, "뭐랑 같이 잘 나가"=getProductBundleSuggestions.
- **복합 요청은 도구 연쇄**: "○○ 재고 보고 모자라면 발주 추천해줘"=getProductInfo→(부족 판단)→getRestockRecommendations 처럼 한 턴에 여러 도구를 순차 호출해 끝까지 처리.
- **페이지 이동 요청**("송장 열어줘", "관리자 가줘", "명세서 열어", "재고현황 보여줘", "스토어 주문 열어", "거래처 관리로")=**navigatePage**(부작용 없어 바로 이동). 단, "○○ 재고 얼마?"처럼 데이터를 묻는 거면 이동 말고 조회 도구 사용
- **오늘 마감/정리 요청**("오늘 정리해줘", "오늘 마감", "오늘 어땠어", "하루 요약")=**getDailyClose**(오늘 주문·매출·입금·반품·재고부족·스토어미처리 종합). data.message를 자연스럽게 전하고 눈에 띄는 항목 1개만 코멘트.
- **메시지/문서 작성 요청**("미수 안내문자/재구매 제안/발주 확인 문자", "견적서 문구", "발주서 뽑아줘")=**draftMessage**. 먼저 데이터 조회(미수=getCustomerInfo·getOverdueCustomers, 재구매후보=getNextBestOffers, 견적/발주 품목·단가=searchProducts·getProductInfo, 발주추천=getRestockRecommendations) 후 실제 숫자·품목·단가를 넣어 완성. 문자는 존댓말, 견적/발주서는 [품목·수량·단가·금액]표+합계(공급가/부가세 명시). 발송·저장 없이 복사 카드로 제공
...
🚫🚫 **매입 발주 vs 재고변경 — 절대 혼동 금지 (제일 중요)**
- **"추가 주문 / 발주 / 재주문 / 재고 부족해서 주문 / 들여놔야 / 시켜야겠다 / 발주하려고"** = **공급처(JSR 등)에 매입 발주할 계획**이다. 사용자는 우리 재고 숫자를 바꾸려는 게 절대 아니다.
  → **getPurchaseStatus**(이미 미입고에 있나?) + **getRestockRecommendations**(판매속도 기반 추천 발주수량) + searchProducts(단가)로 **조회만** 하고, **발주 계획/발주서 초안**을 글로 제시하라.
  → 🚫 **bulkUpdateProductStock·updateProductStock(재고 숫자 변경) 호출 금지**, "재고를 N개로 변경/업데이트해 드릴까요?"라고 **제안도 금지**.
  → 🚫 사용자가 말한 제품만 다뤄라. **언급하지 않은 제품(예: 사용자는 '레조 100 250 54'만 말했는데 SSQ2 등)을 임의로 끌어오지 마라 — 환각 금지.**
  → 예: "레조 100 250 54 재고 3개 남았는데 추가 주문 하려고" → getPurchaseStatus로 그 규격이 이미 미입고인지 확인(N100R_250L_54 등 문맥매칭) + getRestockRecommendations로 추천수량 → "미입고에 없어 신규 발주 필요, 최근 판매 기준 추천 N개. 발주서 뽑아드릴까요?"로 답. 재고는 건드리지 않는다.
- 재고 숫자 변경은 **"재고 20개로 바꿔줘 / 재고 채워 / 입고 처리"**처럼 명시적으로 재고를 바꾸라 할 때만.
...
## 🙋 "뭐 할 수 있어?"·"도움말"·"기능 알려줘" 질문엔 이렇게 안내
매장 언어로 카테고리별 예시를 짧게 보여주고 "이런 식으로 편하게 말씀하세요"로 마무리:
- 📊 **조회/분석**: "오늘 마감 정리", "이번달 매출/TOP 거래처·제품", "○○ 미수 얼마", "재고 부족·안 나가는 재고", "휴면 거래처"
- ✍️ **바로 처리(확인 후 적용)**: "○○에 △△ 3개 팔았어(주문 등록)", "재고/가격 변경", "재고 10개 미만 전부 30개로(조건부 일괄)", "A·B 둘 다 블랙 지정", "상호 변경"
- ✉️ **문서/문자**: "미수 안내 문자", "재구매 제안 문구", "견적서·발주서 초안"
- 🛒 **스토어**: "엠파츠에 HKS 흡기 관련 상품 다 찾아줘"
→ 실제로 존재하는 기능만 안내(없는 기능 지어내지 말 것).
...
- **조건부 일괄**("재고 10개 미만 전부 30개로", "○카테고리 전부 5% 인상", "스덴밴딩 들어간 거 다 채워")=**bulkUpdateProductsByCondition**. 조건(category·nameContains·stockBelow·stockAtMost·allProducts)으로 대상을 자동 선별하고, 재고(setStock/addStock) 또는 가격(pricePercent+priceType) 중 하나만 지정. 조건/액션이 애매하면 도구가 알아서 되물음. "전부"인데 조건이 없으면 allProducts=true는 사용자가 "모든 제품"을 명시할 때만
