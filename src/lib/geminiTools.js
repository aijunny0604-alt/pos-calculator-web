...
export const GEMINI_TOOLS = [
  ...
  {
    name: 'draftMessage',
    description: '거래처에게 보낼 문자/카톡 메시지 초안을 작성해 복사 가능한 카드로 보여줍니다. "○○한테 미수 정산 안내문자 만들어줘", "재구매 제안 문구 써줘", "발주 확인 문자 써줘" 같은 요청에 사용. 반드시 먼저 필요한 데이터를 조회(미수액=getCustomerInfo/getOverdueCustomers, 재구매후보=getNextBestOffers/getCustomerProductAffinity, 회수문구=getCollectionPlan)한 뒤, 실제 숫자·이름을 넣어 정중한 한국어 메시지를 message에 완성해 전달. DB·발송 없음(사장님이 복사해 직접 보냄). 존댓말, 매장 이미지에 맞게 정중하고 간결하게.',
    parameters: {
      type: 'object',
      properties: {
        recipientName: { type: 'string', description: '받는 거래처/고객 이름 (선택)' },
        purpose: { type: 'string', description: '용도 라벨 (예: 미수 정산 안내, 재구매 제안, 발주 확인)' },
        message: { type: 'string', description: '완성된 메시지 본문. 실제 숫자·이름 포함, 정중한 존댓말 한국어. 이 텍스트가 그대로 복사됨. 이모지/마크다운 없이 문자로 바로 보낼 수 있게.' },
      },
      required: ['message'],
    },
  },
...
- **메시지/문자 작성 요청**("○○한테 미수 안내문자/재구매 제안/발주 확인 문자 만들어줘")=**draftMessage**. 먼저 필요한 데이터(미수액=getCustomerInfo·getOverdueCustomers, 재구매후보=getNextBestOffers·getCustomerProductAffinity)를 조회한 뒤, 실제 숫자·이름을 넣어 **정중한 존댓말 한국어 문자**를 message로 완성해 호출. 이모지/마크다운 없이 바로 보낼 수 있게. (발송은 안 하고 사장님이 복사해서 보냄)
