...
const WRITE_INTENT_PATTERNS = [
  ...
  { re: /(재고|수량|개수|재고량)[\s\S]{0,12}(변경|수정|바꿔|입고|출고|들어왔|나갔|업데이트|로\s*해|으로\s*해|정정|채워|맞춰|로\s*맞|개로|개\s*남)|재고\s*없음|품절(로|\s*처리)?\s*(변경|수정|해|로)|소진/, tools: ['updateProductStock', 'bulkUpdateProductStock', 'bulkUpdateProductsByCondition'] },
  // 조건부 일괄 — "재고 N개 미만/이하 전부/다", "○카테고리 전부 M개/%", "X 들어간 거 다"
  { re: /(전부|모두|모든|다\s|싹|일괄)[\s\S]{0,20}(재고|수량|가격|단가|인상|인하|개로|채워|맞춰|%|퍼센트|프로)|(미만|이하|이상|초과|밑|넘는)[\s\S]{0,20}(전부|모두|다|재고|채워|맞춰|개로)|카테고리[\s\S]{0,20}(전부|모두|다|인상|인하|%)/, tools: ['bulkUpdateProductsByCondition', 'bulkUpdateProductStock', 'bulkUpdateProductPrice'] },
  ...
  { re: /(가격|금액|단가|도매가|소매가|소비자가|판매가)[\s\S]{0,25}(변경|수정|바꿔|올려|내려|업데이트|인상|인하|로\s*해)|(변경|수정|바꿔|올려|내려|인상|인하)[\s\S]{0,25}(가격|금액|단가|도매가|소매가|소비자가|판매가)/, tools: ['updateProductPrice', 'bulkUpdateProductPrice', 'bulkUpdateProductsByCondition'] },
