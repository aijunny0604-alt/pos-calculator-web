...
- **묵은 기간 추적**([purchaseExport.js](src/lib/purchaseExport.js) `daysSince`/`ageLevel`): 발주 후 경과일. 30일↑ 주황, **90일↑ 빨강 + 행 배경 강조**. 요약 카드에 "가장 오래 묵은 미입고"(현재 64일). JSR이 물건을 늦게 보내 나중에 헷갈리는 게 실제 고충이라 이게 핵심 신호
...
- **내보내기**([purchaseExport.js](src/lib/purchaseExport.js)): 상태/금액 계산을 **단일 소스**로 두고 화면·CSV·엑셀·프린트가 공유(출력물이 화면과 어긋나지 않게)
  - 📋 카톡복사(미입고 탭): 오래 묵은 순 + "64일 경과" + 🚨/⚠️ 마크 + 관리번호 → JSR에 그대로 전달
...
- 현재: 빌려준 것 68개(6건) / 미입고 164개(11건) / 불량품 29개(12종) / **가장 오래된 건 267일**(CFK 93 D-G 5세트는 203일째 미회수)
...
### 📊 이관 결과 (라이브)
- 발주 **20건 / ₩109,215,095**, 증빙 20/20 연결. 시트엔 10건뿐이었고 **2025-08~12 이력 8건 + 6/15 + 7/15**가 발주서에서 추가로 나옴
- 기존 10건은 **금액 1원도 안 바뀜**(규격명만 발주서 표기로 정정). 발주서 21장 전건 합계 검산 일치
- 미입고 8품목 ₩8,306,000. 7/15 발주서의 NPK89D_B 20개 무상출고 = 5/12 미입고분 → 사장님 수령 확인 후 40/40 완료 처리
- 증빙 원본: Storage `product-images/purchase-quotes/{관리번호}.png` 21장
