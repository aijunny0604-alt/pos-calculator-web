...
- 🔀 **불량품 두 곳 역할 구분**(사장님 지적으로 정리, 둘 다 유지): **[JSR 수불 장부]→불량품 탭**=발생 즉시 품목별 **누적 재고**(supplier_ledger kind=defect), **[매입 발주]→불량 반품 탭**=실제 **발송 묶음+교환 회수**(defect_returns). 양쪽에 서로 가리키는 안내 배너. 수불장부 불량품 행에 **완료 스위치 추가**(발송 처리 시 정리, resolved 플래그, 누적 집계서 제외).
...
- 🆕 **체크 진입점 3곳 동기화** (2026-07-30, 사장님 "제품목록에도 표기"): ①[📦 포장 체크] 버튼(진행 중이면 "📦 포장 N/M") ②**상시 진행도 바**(패널 안 열어도 주문상품 아래 게이지+N/M, 탭하면 패널 열림 — `packedSet.size>0||packingDone`일 때만) ③**주문상품 표 각 행의 No. 자리가 탭 가능한 포장 체크**(미완료=번호/완료=초록 ✓, 모바일은 `✓ 포장` 칩). 셋 다 `togglePacked`/`packedSet` 단일 상태 → DB·이벤트·실시간으로 완전 동기화. 편집모드(isEditing)에선 체크 숨김·번호만.
...
매입 발주에 **[재주문 리스트]** 탭 신설. 재고 0 제품을 판매 실적으로 우선순위 매겨 발주로.
- 대상: `products` 중 `stock=0 & stock_status!=incoming`. `PurchaseOrders`에 `products`·`orders` prop 전달
- **판매 집계 `salesStats`**: 주문 항목 **제품 id로 매칭**(🚨 이름 매칭 금지 — 금액 2배 사고 이력, id 없는 옛 주문만 이름 폴백)
  - **반품 차감**: `orders.returns`의 `itemId`별 반품수량을 빼서 순 판매로 (flow-check 2026-07-24 발견·수정)
  - 기간 30/90/180/365일, 정렬 많이팔린순/최근순/이름순, '판매된 것만' 토글
- **분석 요약 4카드**: 재고0 / 🔥지금 못 파는 중(최근7일 판매+재고0) / 최근N일 이 제품 매출 / 판매이력 없음
  + 급한 것 [이것만 선택] 원클릭 → 품명 채워진 발주서 (규격 입력 시 단가표 자동채움)
- 검증: UI 통계값 == Supabase 독립 재계산 완전 일치(재고0 58·판매36·TOP5·urgent7)
...
### 🔍 매입 발주 검색을 제품주문 로직으로 통일 (`PurchaseOrders.jsx`)
- `toLowerCase().includes()` 단순 부분일치 → `@/lib/utils`의 **`matchesSearchQuery`** 재사용
- 띄어쓰기/하이픈/언더바 무시 + 순서 일치(부분수열) + 다단어 AND
- `TVB 54` → `TVB54h_L_C`, `머플러 원형` → `머플러원형(흡음)`, `FX R` → `FXR` 매칭
- 발주번호/매입처/제목/품목(품명+규격) **4곳 모두** 적용

### 🏷 발주 상태 필터 — 부분입고 / 특이사항
- 검색창 아래 칩: **전체 / 미입고 / 부분 입고 / 완료 / 특이사항** (각 건수 표시)
- 판정은 기존 `poStatus()` 재사용 → 카드 배지·좌측 색띠와 동일 기준
- **특이사항** = `status_override`(주문 취소 등 직접 입력) 또는 **수량 음수**(차감).
  `poSpecialItems` / `poSpecialLabel` 을 `purchaseExport.js`에 추가
- 🚨 **왜 필요했나**: `poStatus`/`poOpenItems`가 이 두 종류를 계산에서 빼기 때문에,
  **취소건만 남은 발주가 목록에서 '완료'로만 보였다**. 실데이터 20건 중 5건이 그 상태였음
- 카드에 사유 배지 노출 — 사유 1종이면 그 문구(`10월 주문 취소건`), 여러 종이면 `특이 N`
- 칩은 **발주 목록 탭 전용**. 미입고 현황 탭은 검색만 반영(상태 골라둔 채 탭 옮겨 빈 목록 되는 것 방지)
- CSV/엑셀/프린트는 필터 결과만 + 안내문에 적용 조건 표기

### 🔔 스마트스토어 메뉴 배지가 백그라운드 탭에서 안 갱신되던 버그 (`App.jsx`)
- 증상: 새 주문이 들어와도 배지 0. 스토어 페이지에 들어가야 그때 숫자가 뜸
- 원인: 배지가 **60초 `setInterval` 하나에만** 의존 → 크롬 Tab Freezing이 타이머를 얼림.
  바로 아래 `handleVisibility`(탭 복귀 갱신)가 이미 있었는데 **주문/상품/거래처/장바구니만 챙기고 배지는 빠져 있었음**
- 수정: 배지 fetch를 `refreshSmartstoreCount`로 분리해 웹소켓 effect 위로 이동 →
  마운트 / 60초 폴링 / **탭 복귀** 3경로. 탭 복귀는 30초 쓰로틀 **밖**에 둠(깨어난 직후가 가장 낡음)
- `SmartStoreOrders`에 `onPendingCountChange` 추가 — 페이지 `reload()`가 배지와 **동일 쿼리**(limit 200, 날짜 무관)를
  쓰므로 추가 요청 0으로 카운트 전달. 전환/발송 직후 배지가 60초 묵던 것 해소
- `catch {}` → `console.warn`. 배지 0이 "주문 없음"인지 "조회 실패"인지 구분 가능하게

### 💳 입금대기를 '결제완료'로 세던 문제 + 전환건 단계 누락 (`SmartStoreOrders.jsx` `orderStage`)
- 진행 단계 위젯이 **입금대기를 '결제완료' 칸**에 넣어 같은 화면 상태 배지와 모순
- **입금 여부를 단계와 독립된 축(`unpaid`)으로 분리**. 무통장은 입금 전에도 발주확인이 되므로
  (`PENDING_CONFIRM_STATUSES`에 `PAYMENT_WAITING` 포함) "발주확인까지 했지만 돈은 안 들어온" 상태가 실재.
  단계에 흡수시키면 그 사실이 사라진다 → 발주확인 칸과 입금대기에 **양쪽 집계**
- 입금대기 + 진행 없음 = `stage -2`(퍼널 밖)
- 🚨 **발주확인 판정에 `internal_order_id` 추가**. 2026-06-15에 전환 시 `order_status`를 `converted`로
  덮지 않게 바뀌면서, status만 보던 이 함수가 **전환건을 시작점에 그대로 두던 드리프트**
  (`isOrderPending`은 그때 고쳐졌는데 `orderStage`만 안 따라감)
- 표시 강화: 입금대기 배지 흐린 회색 → **앰버**, 진행 단계 위에 `입금대기 N건` 바(클릭=필터 `unpaid`),
  카드 스텝퍼 상단 경고 띠, 조회 요약줄에 건수

### 📦 방문수령 주문도 택배 발송처리 가능하게
- 손님이 "그냥 택배로 보내달라"고 바꾸는 경우가 실제로 있는데, `isVisitReceiptOrder`면
  발송 버튼이 방문수령 버튼으로 **스왑**돼 송장 입력 자체가 불가능했다 (리스트/컴팩트/카드 3곳 전부)
- **스왑 → 병기**. 방문수령 주문은 `[방문수령 처리]` + `[발송처리]` 둘 다 노출
- 발송 모달에 방문수령 안내 배너(오조작 방지)
- 발송 로직은 배송수단과 무관하게 동작해 변경 없음. sync.js는 `company_code`가 `VISIT_RECEIPT` 센티널이 아니면 기존 택배 경로

### 🏢 주문 주소/연락처 수정 시 거래처 원장 동기화를 **동의받고** 실행
- 증상: 주문내역서에서 업체 주소를 고쳐도 거래처 정보가 안 바뀜
- 원인: 동기화 코드는 있었으나 **이름 완전일치**로만 거래처를 찾아 매칭 실패 시 조용히 no-op + 성공/실패 무알림
- `handleUpdateOrder(id, data, opts)` — `opts.syncCustomer` `true`=변경 / `false`=이 주문만 / **생략=기존 자동동기화**
  (주문상세 외 호출부 6곳 동작 유지)
- OrderDetail 저장 확인 모달에 **체크박스** + 거래처 현재값→새값. 기본 ON.
  체크 해제 = 이 주문에만 (일회성 배송지가 거래처 기본주소를 덮어써 다음 주문이 엉뚱한 데로 가는 사고 방지)
- 거래처 목록에 없는 이름이면 **그 사실을 모달에 명시** — "왜 안 바뀌지"의 답을 화면에서
- 거래처 값이 이미 같으면 안 물어봄 / 동기화 성공·실패 토스트

### 🎨 모달 5종 리디자인 — 톤 통일
톤 기준은 **`CustomerDetailModal`**. 4단 구조로 통일:
상단 1.5px 그라데이션 액센트 바 → 그라데이션 아이콘 타일(11×11)+제목+부제 헤더 → 스크롤 본문 → 고정 푸터.
`rounded-2xl` + `shadow-[0_25px_80px_-15px_...]` + `animate-modal-up`, 모바일은 하단 시트(`rounded-t-3xl`).

| 모달 | 색 | 주요 개편 |
|---|---|---|
| 발송 처리 | 네이버 그린 | `max-w-md`→`lg`, 주문 요약 카드, **송장 입력을 주인공으로**(text-lg mono, autoFocus, Enter 등록, 입력 시 그린 전환), 자주 쓰는 택배사 2곳 칩 |
| 방문수령 | 앰버 | 경고를 `AlertTriangle` 카드로 분리 |
| 일괄 발송 | 그린 | 헤더에 `선택 N건 · 송장 입력 M건` 상시, 입력된 행만 좌측 그린 띠 |
| 주문 취소 | 레드 | 주문 요약 카드, 판매자센터 링크 버튼형, 사유 프리셋 3열 칩 |
| 주문 수정 확인 | 앰버 | 변경 항목을 **좌측 색 띠 + 태그** 행으로(추가=녹/삭제=적/수량단가=앰버/필드=중립/합계=청) |

- 주 버튼은 미충족 시 이유를 라벨로 (`송장번호를 입력하세요` / `취소 사유를 입력하세요`)
...
- `9f59ff2`: 이미지가 금액·매칭배지 가림 → **`[정보][이미지110][금액]` 3단**으로 재배치. 카드·컴팩트·상세 통일

### 📎 발주서 드래그드롭·Ctrl+V + 발주일 조회 (`12bb947` `4f250b7`)
- 발주서 등록 3경로: 클릭 업로드 / **이미지 드래그드롭** / **Ctrl+V 붙여넣기**.
  `onPickQuote` → `scanQuoteFile(file)` 공통 함수 분리, 페이지 드롭존 오버레이 + window paste 리스너(판독중·모달 시 무시)
- 버튼에 점선 테두리 + "클릭·드래그·Ctrl+V" 명시 — 기능은 있었으나 툴팁뿐이라 사장님이 인지 못 함
...
사장님이 구글시트 7/15 블록을 고치고 **발주서를 교체**(`20260715-4` → `20260714-7`). DB 재적용함.

- **변경**: 4품목 ₩2,112,000 → **5품목 ₩2,024,000**. `CH_250L_61`(33,000×20, 부분입고 10/20) 추가, `N100R_250L_61`은 **단가 0**으로
- 🚨 **"주문했는데 견적서에 없음"** 케이스: `N100R_250L_61`은 주문했으나 JSR 견적서에 미등재 → **청구 안 됨**. 우리 모델은 공급가액=단가×수량이라 표현이 안 됨 → **단가 0 + 비고에 정가(₩37,400) 명시**로 처리. 합계가 시트와 맞음
- 시트 `남은수량` 오타 발견(20 주문 5 입고인데 남은 0) → 사장님 확인 후 **15**로 정정. 같은 패턴의 `CH_250L_61`은 시트가 맞았음(남은 10)
- **총액 ₩109,215,095 → ₩109,127,095 / 미입고 ₩8,306,000 → ₩7,068,500**

### 🔁 시트가 바뀌었을 때 대응 패턴 (재사용)
1. **xlsx 재추출 → 이미지 md5 해시로 대조**. 파일명은 이미지 삽입 시 전부 재번호되므로 **파일명 매칭 금지**. 해시로 "사라진 것/새로 들어온 것"만 뽑으면 정확
2. CSV 재추출해 **블록별 합계만 뽑아 DB와 대조** — 시트 구조(일자 열, 블록 제목)가 수시로 바뀌므로 헤더 파싱에 의존하지 말 것. 실제로 이번에 사장님이 일자 열을 지우고 오래된 블록 제목을 없앰
3. 애매한 셀(계산이 안 맞는 값)은 **추측 금지, 물어볼 것**. 돈이다
4. dry-run으로 합계 검산 → 시트와 일치할 때만 적용
...
### 🔔 예약일 알림 띠 ([ReservationAlertBar.jsx](src/components/ReservationAlertBar.jsx) 신규, [App.jsx](src/App.jsx) AppLayout 안 최상단)
사장님: "오늘이 예약된 날짜일 경우 아직 주문 확인 안된 상태면 까먹지 않게 수시로 알람 뜨게"
- **저장 장바구니에 남아 있다 = 아직 주문 아님**. 예약일(`delivery_date`)이 됐는데 남아 있으면 까먹은 것 → 손님이 물건 못 받는 사고
- **팝업 아니라 띠**: 팝업은 작업을 끊어서 습관적으로 닫게 됨 → 띠는 계속 보이되 작업 안 막음
- **[30분 뒤에] 스누즈**(localStorage `pos_reservation_snooze_until`) — 잔소리가 되면 사람은 알림을 무시하기 시작함
- **당일 + 지난 건**까지. 지난 건 있으면 띠 전체 빨강 격상 + 항목에 ⚠️ (놓친 게 제일 급함)
- 5분 폴링(스누즈 만료 + 다른 기기 변경 반영). 업체명 클릭/[보러가기] → 저장된 장바구니
- 🚨 **fixed 패널 함정**: 제품주문(pos)은 우측 장바구니가 `fixed w-[400px]`([MainPOS.jsx](src/pages/MainPOS.jsx):886)라 띠가 그 아래 깔려 **버튼이 가림** → `reserveRightGutter` prop으로 본문과 동일하게 `lg:pr-[420px]` 보정 + flex-wrap. **전역 띠/배너 추가 시 이 화면 반드시 확인**

### 💰 빠른추가 칩 → 금액 즉시 수정 ([QuickItemBar.jsx](src/components/ui/QuickItemBar.jsx))
사장님: "택배비 퀵비 금액 수정 바로 가능하게. 지금은 주문 수정해서 해야해"
- 칩 클릭 → 바로 담지 않고 **금액 확인창**. 기본값 채운 채 autoFocus+select → 덮어쓰고 **Enter=추가 / Esc=취소**
- 택배비는 건마다 다름(선불/착불/지역)이라 기본값 그대로 쓰는 경우가 드물다. 기존 3단계(담기→주문수정→단가) 제거

### 🔍 UI 확대 (같은 원인 — 열이 많아 카드가 좁으니 폰트를 줄여야 했다)
- **저장 장바구니 목록**([SavedCarts.jsx](src/pages/SavedCarts.jsx)): 3열→2열(2xl만 3열), 업체명 lg→xl black, **[불러오기]/[주문확인] h-11 + text-sm bold**(터치 44px 룰), 삭제 w-11 h-11, 품목/메모 xs→sm, 배송예정일 라벨 10px→xs bold+pill(알림 띠와 직결되는 정보)
- **MainPOS 장바구니 카드 버그 2건**: 택배비에 `공급 4,091`(비과세인데 /1.1) → 전액 공급가로. 택배비에 `잔여 49`(부대비용은 재고 개념 없는데 기본값 50-1) → `비과세` 배지로 대체
...
사장님이 구글시트로 관리하던 **JSR 매입** 전체를 프로그램으로 이관. 시트 3개 탭(발주표·발주서 이미지 21장·빌려줌/불량품)이 전부 들어옴.

🚨 **용어 주의**: 이 문서 대부분의 "발주"는 **네이버 스마트스토어 "발주확인"(판매 주문상태)**이다. 이번에 생긴 **"매입 발주"는 매입처(JSR)에 우리가 발주하는 것**으로 정반대 방향. 코드/DB엔 방어 주석이 있으나 헷갈리기 쉬움. MOVIS WRITE_INTENT에 `/발주/` 정규식 쓰면 충돌 → 부정 lookahead 필수.

### 📦 매입 발주 ([PurchaseOrders.jsx](src/pages/PurchaseOrders.jsx) 신규, 사이드바 `매입 발주` / `case 'purchase-orders'`)
- **테이블 `purchase_orders`** (마이그 [008](../naver-sync-bridge/migrations/008_purchase_orders.sql) **적용완료**): `po_number` 비즈니스키(PO-YYMMDD) + `items JSONB` + RLS anon FOR ALL
  - items: `{name, spec, unit_price, qty, received_qty, note, status_override}`. **qty 음수 허용**(취소분 차감 행)
  - **상태는 저장 안 하고 프론트 계산** — 시트 수식 재현: 입고0=미입고 / 입고<수량=부분입고 / else 완료. `status_override` 있으면 그게 우선(시트의 수동 "주문 취소" 재현)
  - 🚨 **`status_override`와 음수 수량은 `poStatus`/`poOpenItems` 계산에서 빠진다** → 취소건만 남은 발주가 목록에서 '완료'로만 보임.
    그래서 **특이사항 칩**(2026-07-22)이 따로 있음. `poSpecialItems`/`poSpecialLabel` 참조
  - 🚨 **공급가액 = 단가 × 발주수량** (입고수량 아님). 시트와 동일해야 합계가 맞음
- **묵은 기간 추적**([purchaseExport.js](src/lib/purchaseExport.js) `daysSince`/`ageLevel`): 발주 후 경과일. 30일↑ 주황, **90일↑ 빨강 + 행 배경 강조**. 요약 카드에 "가장 오래 묵은 미입고"(최고령 = 2026-05-12 발주분). JSR이 물건을 늦게 보내 나중에 헷갈리는 게 실제 고충이라 이게 핵심 신호
- **빠른 입고**: 미입고 탭 [입고] → 수량 입력 → 확정. 금액 불변, 입고수량만 증가. 남은수량 초과 방어. 🆕 미입고 현황 표의 **규격명 폰트 확대**(text-base~lg 굵게, JSR 전달용 가독성, 2026-07-30 [PurchaseOrders.jsx](src/pages/PurchaseOrders.jsx) 미입고 행)
- **증빙 원본 열람**: 모달 헤더 [발주서 보기] → 견적서 이미지 오버레이. 견적서 2장인 발주(PO-260119)는 1/2 탭 전환
- **내보내기**([purchaseExport.js](src/lib/purchaseExport.js)): 상태/금액 계산을 **단일 소스**로 두고 화면·CSV·엑셀·프린트가 공유(출력물이 화면과 어긋나지 않게)
  - 📋 카톡복사(미입고 탭): 오래 묵은 순 + "N일 경과" + 🚨/⚠️ 마크 + 관리번호 → JSR에 그대로 전달
  - CSV(BOM 붙여 엑셀 한글 안 깨짐) / 엑셀(exceljs **동적 import** — 939kB, 버튼 누를 때만 로드. 발주+미입고 2시트) / 프린트(앱 CSS와 충돌 없게 **별도 창에 A4 전용 HTML**)

### 📷 발주서 사진 → 자동 판독 ([quoteVision.js](src/lib/quoteVision.js) + [QuoteScanModal.jsx](src/components/purchase/QuoteScanModal.jsx) 신규)
- 헤더 **[📷 발주서 사진 등록]** → 무료 gemini flash vision 판독 → 확인 모달 → 등록. 실제 견적서 21장 판독 경험을 프롬프트에 반영(더미행/0원 무상보전/음수 취소차감/날짜 2개 섞임/규격 대소문자). temperature 0
- **자동 저장 절대 안 함** — 매입 증빙이라 사장님이 보고 고치고 [등록] 눌러야 들어감. 모든 칸 수정 가능 + 원본 사진을 옆에 띄워 대조
- `normalizeQuote`가 **산술 자기검산**(행별 단가×수량 vs 인쇄값, 합계 대조) — 판독을 그대로 믿지 않음. 관리번호 중복/흐린 행(uncertain) 경고
- **무상보전 자동연결**: 0원 "미출고품 출고" 행 감지 → `findFillTargets`로 채울 발주 후보(오래 묵은 순) select → 등록 시 그 발주 입고 처리 + 비고에 출처 기록
- 🚨 **Gemini 임베드 키는 referrer 제한** → localhost에서 403. **판독 검증은 배포본에서만 가능**

### 🏷️ 매입 단가표 ([SupplierPrices.jsx](src/pages/SupplierPrices.jsx) 신규, 사이드바 `매입 단가표` / `case 'supplier-prices'`)
- **테이블 `supplier_prices`** (마이그 [009](../naver-sync-bridge/migrations/009_supplier_prices.sql) **적용완료**): 규격별 단가 이력 75행/고유 규격 60종. UNIQUE(supplier_name, spec, quoted_at, unit_price)
- ⚠️ **여긴 JSR 매입가**(우리가 지불) — `products.wholesale`(업체에 파는 도매가)과 **완전 별개**. 규격코드(`NPK89D_B`) vs 제품명(`CH 200 61`)이라 이름 체계도 다름
- 도출 규칙: 더미행(자동차_부품) 제외 / **단가 0원 제외**(미출고품 무상출고 보전이라 단가가 아님) / 수량 음수 제외
- 행 클릭 → 단가 이력 펼침 + 구간별 변동률. **단가 인상 10종 확인**(예: NPK89D_B 2025-10 ₩51,480 → 2026-05 ₩58,300 +13.2%). 📋 인상내역 카톡복사
...
### 📄 사업자등록증 노출 ([CustomerDetailModal.jsx](src/components/CustomerDetailModal.jsx) + [CustomerList.jsx](src/pages/CustomerList.jsx))
- **결제/주문이력 모달**: 아바타(🏢) 자리를 등록증 썸네일로 교체 + 이름 옆 [사업자등록증] 배지 → 클릭 시 원본 오버레이. 없으면 기존 🏢. 이 모달은 거래처관리 [결제 상세]/대시보드 미수/명세서에서 열림
- **거래처 목록 카드**: 연동된 업체만 좌측 썸네일 → 목록에서 바로 구분. 기존 `certViewer` 상태/뷰어 재사용
- 데이터는 `customers.business_cert_url` 재사용(등록증 보관함이 매칭해둔 36건, 그중 미수 보유 18건)

### 🎨 UI 정리
- **주문 목록(장바구니) 카드 2열→1열**([MainPOS.jsx](src/pages/MainPOS.jsx)): 400px 패널에 2열이라 카드가 190px → 폰트를 9~12px까지 줄여야 했던 게 원인. 1열로 폭 2배 확보 후 제품명 16px/금액 24px/공급가 12px, 수량 버튼 24→36px
- **저장 장바구니 모달**([SavedCarts.jsx](src/pages/SavedCarts.jsx)): [제품 추가]를 보기 모드에도 노출 → 1클릭에 편집진입+검색창+포커스([수정]→[제품 추가] 2단계 제거). 창 1200×820 → **1520×940**. ⚠️ 크기가 localStorage에 저장되는 구조라 기본값만 바꾸면 안 먹음 → 키를 `pos-web.savedCartDetailModal.v2`로 올림
- **모달 어수선함 정리**: [저장]이 헤더·푸터 중복 → 헤더는 "편집 중" 배지로, 저장은 푸터 하나로. [교체]/[삭제] 테두리 제거해 고스트 버튼화. [QuickItemBar.jsx](src/components/ui/QuickItemBar.jsx) 카드 박스 제거(공유 컴포넌트라 주문상세/주문등록도 함께 정돈)
- **스토어 카드 파란 빛 제거**([index.css](src/index.css)): `.order-hover-premium`이 hover만 해도 파란 테두리+글로우 2겹+대각선 sweep(0.75s)+3px 들썩임 → 은은한 회색 그림자만. `.order-row-premium` 청록 글로우 바도 제거. 부수효과로 블랙리스트/클레임 카드 자체 색이 파란 hover에 덮이던 문제도 해결

### 🚨 이번에 배운 함정 (재발 방지)
- **규격명 문자열 매칭 금지**: `TVB64Y L`↔`TVB64Y_L_C`, `CH 150 64`↔`CH_150L_64`, `100 200 64`↔`N100R_200L_64`. 정규화해도 안 맞아 중복행이 생겨 **금액 2배 사고 날 뻔함** → **(단가, 수량)으로 매칭**할 것. "금액 바뀌면 중단" 가드가 DB 건드리기 전에 막아줌 — 데이터 정정 스크립트엔 이런 가드 필수
- **JSX**: 삼항연산자 `(` 바로 뒤 `{/* */}` 주석 블록은 파서 깨짐(`Expected ) but found className`) → `//` 주석 쓸 것
- **공개 저장소 주의**: 커밋 메시지에 실고객 상호·사업자등록번호를 쓸 뻔함(권한 체크에 걸려 차단). 검증 결과 적을 때 실데이터 금지
- **JSR 원본 오류**: 2026-06-15 견적서 40,598×19=771,362인데 공급가액란 771,367(5원 차이). 판독 오류 아니라 원본 문제 → 해당 발주 memo에 기록. JSR 확인 필요

### 📋 신규 파일
```
src/pages/PurchaseOrders.jsx               # 매입 발주 (발주목록·미입고·사진판독·증빙)
src/pages/SupplierPrices.jsx               # 매입 단가표 (규격별 단가 이력)
src/pages/SupplierLedger.jsx               # JSR 수불 장부 (빌려줌·미입고·불량품)
src/lib/purchaseExport.js                  # 상태/금액 단일소스 + CSV/엑셀/프린트/카톡/묵은기간
src/lib/quoteVision.js                     # 발주서 사진 판독 + 자기검산 + 보전 후보 검색
src/components/purchase/QuoteScanModal.jsx # 판독 결과 확인 모달 (신규 디렉터리)
```

### 📊 이관 결과 (라이브)
- 발주 **20건 / ₩109,127,095**, 증빙 20/20 연결. 시트엔 10건뿐이었고 **2025-08~12 이력 8건 + 6/15 + 7/15**가 발주서에서 추가로 나옴
- 최초 이관 시 기존 10건은 **금액 1원도 안 바뀜**(규격명만 발주서 표기로 정정). 발주서 21장 전건 합계 검산 일치
- 미입고 **8품목 ₩7,068,500**. 7/15 발주서의 NPK89D_B 20개 무상출고 = 5/12 미입고분 → 사장님 수령 확인 후 40/40 완료 처리
- 증빙 원본: Storage `product-images/purchase-quotes/{관리번호}.png` **22장**(7/15 교체분 20260714-7 포함)
- ⚠️ **숫자는 라이브를 따름** — 사장님이 시트를 고치면 바뀐다. 문서 숫자가 의심되면 `purchase_orders`를 직접 조회할 것

## 🆕 v2026-07-13 — 제품교체 모달 재설계 · 택배 발송인 버그 · MOVIS 재고리스트 선택
...
- 🚨 **items 저장 시 `taxFree` 반드시 함께 넘길 것** (2026-07-16): 저장 경로들이 `{id, name, price, quantity}`처럼 **필드를 명시 나열**해서 그냥 두면 **저장 순간 비과세가 날아가 과세로 되돌아간다**(화면과 명세서 금액이 어긋남). 현재 보존 지점: [MainPOS](src/pages/MainPOS.jsx) saveOrder · [OrderPage](src/pages/OrderPage.jsx) baseItem · [App.jsx](src/App.jsx) 저장카트. **새 저장 경로 만들면 여기 추가**
- 🚨 **"발주" 두 개가 정반대 방향** (2026-07-15): **네이버 "발주확인"** = 스마트스토어 **판매** 주문상태(구매자 결제 → 우리가 주문접수). **"매입 발주"**(`purchase_orders`) = 우리가 **매입처(JSR)에** 발주하는 것. 이 문서의 "발주" 대부분은 전자. 코드/DB엔 방어 주석 있음([Sidebar.jsx](src/components/layout/Sidebar.jsx), 마이그008 테이블 COMMENT). **MOVIS WRITE_INTENT에 `/발주/` 정규식 쓰면 "발주확인"과 충돌** → 부정 lookahead 필수. 매입 미입고도 두 종류(`purchase_orders`=발주서 건의 입고 잔량 / `supplier_ledger`=발주서 무관하게 오간 것)
...
- `pos_quick_items_v1` — QuickItemBar 부가 항목 프리셋 (택배비/퀵비/수수료 + 사용자 추가). ⚠️ 기존 저장분엔 `taxFree` 필드가 없어서 `preset.taxFree !== false`로 기본 비과세 처리
- `pos_reservation_snooze_until` — 예약일 알림 띠 스누즈 만료 시각 (30분)
- `pos-web.savedCartDetailModal.v2` — 저장카트 상세 모달 크기/위치. ⚠️ 크기가 저장되는 구조라 기본값만 바꾸면 안 먹음 → 키 버전을 올려야 새 기본값 적용
...
| [AI 기능 원리](docs/AI-FEATURES.md) | 발주서·등록증 판독 / 주문 인식 / MOVIS 분석 어시스턴트 작동 원리·안전장치·비용 |
