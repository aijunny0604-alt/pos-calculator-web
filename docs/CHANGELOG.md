...
- 판매 실적 집계(`salesStats`): 주문 항목 **제품 id로 매칭**(이름 매칭 금지 — 금액 사고 이력)
  - 기간 선택 30/90/180/365일, 정렬 많이팔린순/최근순/이름순, '판매된 것만' 토글
  - 카드: 순위배지(1~3위 🔥) + 판매량 게이지 + "N개 판매·N회·N일 전"
- 분석 요약 4카드: 재고0 / 🔥지금 못 파는 중(최근7일 판매+재고0) / 매출합계 / 판매이력 없음
  + [이것만 선택] 원클릭 → 급한 것만 발주서로
- 단가 자동매칭 안 함(판매명 vs 매입 규격명 체계 상이). 규격 입력 시 단가표 자동채움 작동
- App.jsx → products·orders prop 전달

#### 🧪 flow-check 발견 — 반품 미차감 수정 (2026-07-24)
- `salesStats`가 `items.quantity`만 더하고 `orders.returns`를 안 뺐음 → 판매량 과대 가능
- 주문 항목 `itemId`별 반품수량 집계해 **순 판매(주문−반품)** 로 수정
- 현재 데이터엔 재고0 제품 반품 0건이라 순위 변화 없음(회귀 안전), 향후 방어

#### 🖼️ 재주문 외 오늘 배포분 (문서화 완료분)
> 발주 드롭·날짜·필터정리(`12bb947` `4f250b7`), 상품 이미지 3단(`9f59ff2`)은 위 항목 참조
