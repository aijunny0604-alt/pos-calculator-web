...
### 메모 모니터링 (2026-04-17 검증)
- [x] 메모 검색: 검색창에 메모 내용 입력 → 해당 주문 필터링
- [x] 메모 필터 3단계: OFF → 미확인만 → 전체 메모 → OFF 순환
- [x] 미확인 필터 클릭 시 날짜 자동 "전체" 전환
- [x] 메모 체크 토글: 카드에서 체크 버튼 클릭 → DB memo_checked 저장 + UI 즉시 반영
- [x] 체크 버튼 터치 영역 36px+ (모바일 터치 가능)
- [x] 토스트: 주문 내역 진입 시 "미확인 메모 N건" 4초 표시
- [x] 토스트 터치 → 미확인 필터 + 전체 날짜 자동 전환
- [x] 대시보드: 미확인 메모 리스트 표시 + 체크 완료 + 주문 상세 이동
- [x] 대시보드: AI 빠른 주문 입력 → AI 주문 인식 페이지 텍스트 자동 입력
- [x] 전 페이지 콘솔 에러 0건

### 반품 플로우 (2026-04-16 검증)
- [x] 주문 상세 → 반품 버튼 → 수량 선택 → 반품 처리 → DB 저장 (orders.returns + customer_returns)
- [x] 반품 후 주문 상세에서 "반품 내역" 섹션 + "반품 N" 배지 표시
- [x] 반품 후 주문 목록에서 "반품 -금액" 배지 즉시 반영 (새로고침 불필요)
- [x] 수량 0에서 반품 처리 시 "상품을 선택해주세요" 경고
- [x] maxReturnQuantity = 원래 수량 - 이미 반품 수량 (이중 반품 방지)
